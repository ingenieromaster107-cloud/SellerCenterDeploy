...
import { ProductCreateConfigurableView } from './product-create-configurable-view';

jest.mock('@mui/lab/LoadingButton', () => ({
  __esModule: true,
  default: ({ children }: any) => <button type="submit">{children}</button>,
}));
...
jest.mock('src/hooks/product', () => ({
  useConfigurableProductPayload: () => ({ buildPayload: jest.fn() }),
}));
...
jest.mock('src/actions/product/use-attributes-list', () => ({
  useAttributesList: () => ({ attributes: [], isLoading: false }),
}));

jest.mock('src/actions/product/use-create-configurable-product', () => ({
  useCreateConfigurableProduct: () => ({ mutateAsync: jest.fn(), isPending: false }),
}));
...
describe('ProductCreateConfigurableView', () => {
  it('renders home content', () => {
    renderWithTheme(<ProductCreateConfigurableView />);
