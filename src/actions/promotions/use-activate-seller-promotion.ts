...
import { ACTIVATE_SELLER_PROMOTION_MUTATION } from './graphql';
...
export function useActivateSellerPromotion(options: Options = {}) {
  const queryClient = useQueryClient();
  const graphql = GraphQLService.getInstance();

  return useMutation({
    mutationKey: ['sellerPromotion', 'activate'],
    mutationFn: (promotion_id: number) =>
      graphql
        .request<
          { activateSellerPromotion: SellerPromotionMutationResponseRaw },
          { promotion_id: number }
        >(ACTIVATE_SELLER_PROMOTION_MUTATION, { promotion_id })
        .then((res) => res.activateSellerPromotion),
