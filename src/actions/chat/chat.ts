...
import type { ChatMessage, ChatParticipant, ChatConversation } from 'src/interfaces/chat/chat';
...
export async function sendMessage(conversationId: string, messageData: ChatMessage) {
  ...
  mutate(
    conversationUrl,
    (currentData) => {
      const currentConversation: ChatConversation = currentData.conversation;
  ...
  mutate(
    conversationsUrl,
    (currentData) => {
      const currentConversations: ChatConversation[] = currentData.conversations;

      const conversations: ChatConversation[] = currentConversations.map(
        (conversation: ChatConversation) =>
...
export async function createConversation(conversationData: ChatConversation) {
  const url = [CHAT_ENDPOINT, { params: { endpoint: 'conversations' } }];
  ...
  mutate(
    ...
    (currentData) => {
      ...
      const conversations: ChatConversation[] = [...currentConversations, conversationData];
...
export async function clickConversation(conversationId: string) {
  ...
  mutate(
    ...
    (currentData) => {
      ...
      const conversations = currentConversations.map((conversation: ChatConversation) =>
